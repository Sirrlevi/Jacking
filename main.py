...
# Try to import cfonts, install if missing
try:
...
# Default range = 2011 accounts (ID 10000 – 17699999)
START_ID = int(os.getenv("START_ID", "10000"))
END_ID   = int(os.getenv("END_ID", "17699999"))
...
# ---------- Banner ----------
GOJO = render('{NAGI META}', colors=['white', 'blue'], align='center')
print(f'''\n
 INSTAGRAM META HUNTER BY • NAGI PYTHON
        ''')
print(f'\x1b[1;39m━'*60)
print(f"{CYAN}Hunting only 2011 accounts (ID range: {START_ID} – {END_ID})")
print(f"{CYAN}Bot token and chat ID loaded from environment.\n")
print(f'\x1b[1;39m━'*60)
...
─────────────────── 
By @serverown1 & @nagipy '''

    with open('nagi.txt', 'a') as ff:
        ff.write(f'{ss}\n')

    try:
        if BOT_TOKEN and CHAT_ID:
            requests.get(f"https://api.telegram.org/bot{BOT_TOKEN}/sendMessage?chat_id={CHAT_ID}&text={ss}")
    except Exception as e:
        pass
...
def gg():
    while True:
        data = {
            ...
            "variables": json.dumps({"id": int(random.randrange(START_ID, END_ID)), "render_surface": "PROFILE"}),
...
if __name__ == "__main__":
    # Initialise helper files
    tll()
    Getaol()

    # Start 300 threads (adjust if needed)
    for _ in range(300):
